...
WORKDIR /src
COPY ["PoligonosAppAWSLambda.csproj", "PoligonosAppAWSLambda/"]
RUN dotnet restore "PoligonosAppAWSLambda/PoligonosAppAWSLambda.csproj"

WORKDIR "/src/PoligonosAppAWSLambda"
COPY . .
RUN dotnet build "PoligonosAppAWSLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "PoligonosAppAWSLambda.csproj" \
...
Execute unit tests
```
cd "PoligonosAppAWSLambda/test/PoligonosAppAWSLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "PoligonosAppAWSLambda/src/PoligonosAppAWSLambda"
